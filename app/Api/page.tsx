...
//import user from "../server/user.png";
//import bot from "../server/bot.jpeg";
...
const ChatInput = ({ onSend, disabled}: InputProps) => {
    ...
    return (
        <div className=" bg-whiet border-2 p-2 rounded-lg flex justify-center">
            <input
            value= {input}
            onChange={(ev: any) => setInput(ev.target.value)} 
            className="w-full pu-2 px-3 text-gray-800 rounded-lg focus:outline-none"
        ...
        <main className="relative max-w-2xl mx-auto">
            ...
            <div className="mt-10 px-4">
                {messages.map((msg: MessageProps) => (
                    <ChatMessage key={Number(msg.key)} text={msg.text} from={msg.from} />
                ))}
